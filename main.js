...
const channel = new BroadcastChannel("my-driving-skills-input");
...
let best = Number(localStorage.getItem("my-driving-skills-best") || 0);
...
if (view === "control") {
  ...
  badge("🎮 My Driving Skills 컨트롤러 — 양손으로 핸들 조작");
  buildCluster();
} else if (view === "game") {
  document.body.classList.add("view-game");
  badge("🖥️ My Driving Skills 시뮬레이션 창 — 컨트롤러 창에서 조작", true);
...
async function begin() {
  ...
  camStatus.textContent = "카메라 권한 요청 중…";
  try {
...
$("twowin-btn").addEventListener("click", () => {
  ...
  window.open(location.pathname + "?view=control", "my-driving-skills-control", "width=1280,height=1180,menubar=no,toolbar=no");
